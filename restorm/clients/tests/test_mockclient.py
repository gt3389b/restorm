...
from restorm.clients.mockclient import (
    MockClient, StringResponse, FileResponse, MockResponse, BaseMockApiClient
)
...
class MockApiClientTests(TestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response['X-Cache'], self.client.responses['search/']['POST'][0]['X-Cache'])
...
class JSONMockApiClientTests(TestCase):
    ...
    def setUp(self):
        ...
        self.client = MockJSONApiClient(
            responses=self.responses, root_uri='http://www.example.com/api/')
    ...
    def setUp(self):
        self.responses = {
            '{"function": "get_book_list"}': (
                {'Status': 200}, [{'id': 1, 'name': 'Dive into Python'}]),
            '{"function": "get_book", "params": {"id": 1}}': (
                {'Status': 200}, {'id': 1, 'name': 'Dive into Python', 'author_id': 1}),
            '{"function": "get_author_list"}': (
                {'Status': 200}, [{'id': 1, 'name': 'Mark Pilgrim'}]),
            '{"function": "get_author", "params": {"id": 1}}': MockResponse(
                {'Status': 200}, {'id': 1, 'name': 'Mark Pilgrim'})
        }
            ...
            **Example**
            ...
            def get_response_from_request(self, request):
                ...
                # Otherwise, return the headers and content from the responses.
                    ...
                    status_code = response_headers.pop('Status')
